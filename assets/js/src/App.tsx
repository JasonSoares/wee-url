...
function putRecent(newUrl: ShortUrl, existingUrls: ShortUrl[], maxRecent: number = 5): ShortUrl[] {
  existingUrls ||= new Array()
...
const App: React.FC = () => {
  ...
  const {setValue: setFormValue, getValues, control, handleSubmit, resetField, setError: setFormError} = useForm<FormFields>({ defaultValues: { url: "" }})
  const [recentUrls, setRecentUrls] = useLocalStorage<ShortUrl[]>("recentUrls", [])
  const [currentAlert, setCurrentAlert] = useState<AlertProps | null>(null)

  function setSuccessAlert(message: string) {
    setCurrentAlert({ type: 'success', message, onClick: dismissAlert })
  }

  function setErrorAlert(message: string) {
    setCurrentAlert({ type: 'error', message, onClick: dismissAlert })
  }
  ...
  const submitHandler = async ({ url }) => {
    if (formMode === "copy") {
      ...
      try {
        ...
        setFormMode("copy")
        resetField("url")
        setFormValue("url", newShortUrl, { shouldDirty: false, shouldTouch: false, shouldValidate: false})

        setSuccessAlert('Successfully created wee URL!')

        const newRecent = putRecent({ url, shortUrl: newShortUrl}, recentUrls as ShortUrl[])
        setRecentUrls(newRecent)

      } catch (err) {
        setErrorAlert('There was an error handling your request. Please try again later.')
      }
    }
  }
  ...
  return (
    <>
      <Header />
      <div className="container  mx-auto mt-24 px-4 w-full max-w-5xl">
        {currentAlert && <Alert onClick={dismissAlert} {...currentAlert} />}
        <div className="flex items-center w-full justify-center mx-auto">
          <h1 className="text-2xl">Paste a link in the form below to get a wee version of it!</h1>
        </div>
        <div className="flex items-center w-full justify-center pt-6 mx-auto">
          <Form handleSubmit={handleSubmit} control={control} submitHandler={submitHandler} mode={formMode} inputChanged={inputChanged} />
        </div>
        {recentUrls.length > 0 && (
          <div className="flex items-center w-full justify-center pt-6 mx-auto">
            <RecentList shortUrls={recentUrls} onCopy={copyUrlToClipboard} />
          </div>
        )}
      </div>
    </>
